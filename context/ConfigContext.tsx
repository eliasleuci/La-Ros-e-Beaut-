...
export function ConfigProvider({ children }: { children: React.ReactNode }) {
    ...
    const addClientProfile = async (client: ClientProfile) => {
        ...
        await supabase.from('client_profiles').insert({
            ...
            private_notes: client.privateNotes || null,
            ...
            last_service_name: client.lastServiceName || null,
    ...
    const updateInventoryCategory = async (cat: InventoryCategory) => {
        ...
        // Also update category name in items
        setInventoryItems(prev => prev.map(i => i.categoryId === cat.id ? { ...i, categoryName: cat.name } : i));
        await supabase.from('inventory_categories').update({ name: cat.name, description: cat.description }).eq('id', cat.id);
    };
    ...
    const deleteInventoryCategory = async (id: string) => {
        ...
        const itemIds = inventoryItems.filter(i => i.categoryId === id).map(i => i.id);
        setInventoryItems(prev => prev.filter(i => i.categoryId !== id));
        setServiceInventoryCosts(prev => prev.filter(s => !itemIds.includes(s.inventoryItemId)));
    ...
    const deleteInventoryItem = async (id: string) => {
        ...
        setServiceInventoryCosts(prev => prev.filter(s => s.inventoryItemId !== id));
    ...
    const updateServiceInventoryCost = async (link: ServiceInventoryCost) => {
        setServiceInventoryCosts(prev => prev.map(s => s.id === link.id ? link : s));
    ...
    const deleteServiceInventoryCost = async (id: string) => {
        setServiceInventoryCosts(prev => prev.filter(s => s.id !== id));
    ...
    const calculateAnalytics = () => {
        ...
        teamMembers.forEach(member => {
            ...
            // Rough occupancy: bookings / working days (assume 22 days, 8 slots per day)
            const maxSlots = 22 * 8;
